...
func GetUserByID(userID string) (*entity.User, error) {
	...
	err = db.QueryRow("SELECT id, name, email, fav_position, biography, created_at, image_url FROM users WHERE id=$1", userID).Scan(
		&user.ID, &user.Name, &user.Email, &user.Fav_position, &user.Biography, &user.Created_at, &user.Image_url)
	if err != nil {
