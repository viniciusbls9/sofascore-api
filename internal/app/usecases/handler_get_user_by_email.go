...
func HandlerGetUserByEmail(email string) (*entity.User, error) {
	...
	err = db.QueryRow("SELECT * FROM users WHERE email=$1", email).
		Scan(&user.ID, &user.Name, &user.Email, &user.Velocity, &user.Fav_position, &user.Rating, &user.Biography, &user.Created_at)
